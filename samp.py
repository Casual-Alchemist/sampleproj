...
class App(QWidget):
    ...
    def Get_Summaries(self):
        items = ("False", "True")
        item, okPressed = QInputDialog.getItem(self, "INPUT DIALOG BOX","Only_Summaries:", items, 0, False)
    ...
    def Get_Interface(self):
        ...
        if text is None:
            raise Exception("Interface can't be None")
    ...
    def cap(self):
        ...
        capture = pyshark.LiveCapture(interface=interface_val, bpf_filter=bpf_val, display_filter=dis_val, only_summaries=summaries,  decryption_key=None, encryption_type='wpa-psk', output_file=filename, decode_as={'udp.port==57427':'rtp'})
        capture.sniff(timeout=timeout_val)
        print(capture)
        #if len(capture) == 0:
        #    raise Exception("check Interface")

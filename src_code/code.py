...
class App(QWidget):
    ...
    def Get_Outputfile(self):
        text, okPressed = QInputDialog.getText(self, "INPUT DIALOG BOX","Enter filename without extension:", QLineEdit.Normal, "")
        if "." in text:
    ...
    def Get_Choice(self):
        j, okPressed = QInputDialog.getInt(self, "INPUT DIALOG BOX","press 1 to get desired filter\n2 - get all the filters:", 1, 0, 100, 1)
        if okPressed:
            return j

    def Get_Inputfile(self):
        text, okPressed = QInputDialog.getText(self, "INPUT DIALOG BOX","Enter filename with .pcap extension:", QLineEdit.Normal, "")
        if not ".pcap" in text:
            raise Exception("Enter filename with .pcap extension")
        if okPressed and text != '':
            return text
